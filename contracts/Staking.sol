...
contract stakingContract is IdjotErc20, IdjotErc1155 {
    ...
    function updateDepositAddress(address payable _newDepositAddress) public onlyOwner {
    ...
    function stakeNFT(uint256 _nftId, uint256 _nftAmount) public returns(bool){
        ...
        require(balanceOf(msg.sender, _nftId) >= _nftAmount, "You don't have required amount of the specified NFT ID");

        // nftId must not be already staked
        require(stakedInfo[msg.sender].isStaked == false, "NFT is already staked!");
    ...
    function unStakeNft(uint256 _nftId, uint256 _nftAmount) public returns(uint256) {
        ...
        require(stakedInfo[msg.sender].nftId == _nftId, "You don't have any staked item under this id");

        // require the user has staked the specified amount or less
        require(stakedInfo[msg.sender].nftAmount >= _nftAmount, "You have staked lesser value of this id");
